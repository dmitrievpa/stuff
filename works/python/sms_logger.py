...
def gotConnection(conn, username, password):
    ...
    # Bind to deliver.sm.* routes to track incoming messages
    yield chan.queue_bind(queue="sms_logger_queue", exchange="messaging", routing_key='deliver.sm.*')
    ...
    while True:
        ...
        if msg.routing_key[:10] == 'submit.sm.' and msg.routing_key[:15] != 'submit.sm.resp.':
            ...
            cursor.execute("UPDATE submit_log SET status = %s, status_at = %s WHERE msgid = %s;", (
                ...
                props['message-id'],))
            db.commit()
        elif msg.routing_key[:11] == 'deliver.sm.':
            pdu = pickle.loads(msg.content.body)
            # print 'PROPS'
            ...
            # print 'PROPS'
            # print 'PROPS.HEADERS'
            # for key, value in props['headers'].iteritems() :
            #     print key, value
            # print 'PDU.HEADERS'
            # print 'PDU.PDU'
            # dump(pdu)
            ...
            pdu_count = 1
            short_message = pdu.params['short_message']
            source_connector = props['headers']['connector-id']
            routed_cid = msg.routing_key[10:]
